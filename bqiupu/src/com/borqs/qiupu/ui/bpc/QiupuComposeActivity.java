...
//                	}
    ...
    private void showPhotoOrFileFromUri(Uri uri, String mimeType) {
        boolean isImage = mimeType != null && mimeType.contains("image/");
        String extName = FileInfo.getFileType(this, uri);
        if (isImage || isImageFile(extName)) {
            imageFileUri = uri;
        ...
        if (action != null) {
            if (intent.hasExtra(Intent.EXTRA_STREAM)) {
                if (action.equals(Intent.ACTION_SEND_MULTIPLE)) {
                    ...
                    for (Uri uri : list) {
//                        showPhotoOrFileFromUri(uri, null);
                        String extName = FileInfo.getFileType(this, uri);
                ...
                if (action.equals(Intent.ACTION_SEND)){
                    ...
                    showPhotoOrFileFromUri(uri, mime_type);
                }
            } else {
                if (action.equals(Intent.ACTION_SEND)){
...
//    }
    ...
    private boolean isImageFile(String extendedName) {
...
//        mFileMimeType = FileInfo.mimeType(tmpUri);
        mFileMimeType = FileInfo.mimeTypeByExtendedName(extendedName);
...
//            setLocationStatus(true);
    ...
    private void showPhoto(boolean isGallery, Intent intent, boolean isMultiplePhoto) {
        ...
        final ComposeShareData data;
        if(isMultiplePhoto) {
	        data = addDataToList(title, tmpUri, MULTI_SHARE_PHOTO, ComposeShareData.PHOTO_TYPE);
        } else {
            data = addDataToList(title, tmpUri, SINGLE_SHARE_PHOTO, ComposeShareData.PHOTO_TYPE);
        }

        if (null != data && imageFileUri != null) {
            data.mExtendedName = FileInfo.getFileType(this, imageFileUri);
        }

        formatExtraAsComment(intent);
    }
    ...
    private void showOtherFiles(Uri uri, String mimeType) {
        ...
        ComposeShareData data = addDataToList(title, tmpPath, mimeType, ComposeShareData.OTHER_TYPE);
        if (null != data) {
            data.mExtendedName = FileInfo.getFileType(this, uri);
        }
...
//                link_url = bundle.getString(Intent.EXTRA_TEXT);
    ...
    private ComposeShareData addDataToList(String title, String url, String version, int type) {
...
//        }
    ...
    private void showContact(Intent intent) {
        ...
        ComposeShareData data = addDataToList(mContactInfo.display_name_primary, String.valueOf(mContactInfo.mContactId), String.valueOf(mContactInfo.mPhotoId), ComposeShareData.VCARD_TYPE);
...
//                	String username = orm.queryUserName(Long.valueOf(suid));
    ...
    private void sharePhoto(final ComposeShareData data) {
        ...
        asyncQiupu.postPhotoAsync(getSavedTicket(), data.mMessage,
                data.mRecipient, data.mFile, data.mExtendedName, photo_id, "", data.mAppData,
...
//            Toast.makeText(this, R.string.privacy_show, Toast.LENGTH_SHORT).show();
        ...
        asyncQiupu.fileShare(getSavedTicket(), data.mRecipient, data.mMessage,
                data.mAppData, data.mIsPrivate, data.mAllowComment, data.mAllowLike,
                data.mAllowShare, "", "", data.mFile, data.mTitle, data.mExtendedName, data.mScreenShotFile,
...
//    }
    ...
    private void parseFileUri(Intent intent) {
        if (intent != null) {
            ...
            showPhotoOrFileFromUri(uri, mimeType);
        }
    }
...
//    			(int)getResources().getDimension(R.dimen.default_text_padding_top), (int)getResources().getDimension(R.dimen.category_padding_right), (int)getResources().getDimension(R.dimen.default_text_padding_bottom));
    ...
    private void formatExtraAsComment(Intent intent) {

//        shareIntent.putExtra(Intent.EXTRA_SUBJECT, getSummary());
//        shareIntent.putExtra(Intent.EXTRA_TITLE, getSummary());
//        shareIntent.putExtra(Intent.EXTRA_EMAIL, AccountSession.account_email);
//        StringBuilder builder = new StringBuilder();
//        builder.append(getDescription()).append("\n").append(getAuthorName());
//        if (!getAuthorName().equals(AccountSession.account_email)) {
//            builder.append("(Email: ").append(AccountSession.account_email).append(")");
//        }
//
//        shareIntent.putExtra(Intent.EXTRA_TEXT, builder.toString());
        ...
        if (!TextUtils.isEmpty(title) && !filter.contains(title)) {
            builder.append("\nSUMMARY: ").append(title);
            filter.add(title);
        }
        if (!TextUtils.isEmpty(email) && !filter.contains(email)) {
            builder.append("\nE-MAIL: ").append(email);
            filter.add(email);
        }
        if (!TextUtils.isEmpty(text) && !filter.contains(text)) {
            builder.append("\nDESC: ").append(text);
            filter.add(text);
        }

        if (builder.length() > 0) {
            mShareComment.setText(builder.toString());
        }
    }
}
