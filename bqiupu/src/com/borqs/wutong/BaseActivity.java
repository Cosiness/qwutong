...
import com.borqs.qiupu.ui.QiupuFeedbackActivity;
...
public abstract class BaseActivity extends FragmentActivity implements ProgressInterface,
...
//        setupLocationListener();
    ...
    public void updateLocationWithBaiduApi(final BDLocation loc, final String address, final String locString) {
        if (mHandler != null)
            mHandler.post(new Runnable() {
                public void run() {
                    String mapUrl = String.format("<a href='%1$s'>%2$s</a>", BaiduLocationProxy.getInstance(BaseActivity.this).getPureMapsSearchString(
                            BaseActivity.this, loc), address);
    ...
    public void updateLocation(final Location loc) {
        ...
        QiupuORM.sWorker.post(new Runnable() {
            public void run() {
                Address address = LocationRequest.getLocationAddress(BaseActivity.this, loc);
                if (address != null) {
                    final String locationInfo = LocationRequest.getAddressInfo(BaseActivity.this, address);
...
//                        }
    ...
    private void logout() {
        ...
        try {
            Intent service = new Intent(BaseActivity.this, QiupuService.class);
            stopService(service);

            Intent bservice = new Intent(BaseActivity.this, BorqsAccountService.class);
...
//                        title_container.setVisibility(View.GONE);
    ...
    public class ActionsAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup arg2) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(BaseActivity.this).inflate(R.layout.main_tab_more_item, null);
...
//				doActionFriendEndCallBack(msg);
                ...
                case QiupuMessage.MESSAGE_REMOVE_FAVORITES_END: {
                    ...
                    if (msg.getData().getBoolean(RESULT, false) == false) {
                        Toast.makeText(BaseActivity.this, msg.getData().getString(ERROR_MSG), Toast.LENGTH_SHORT).show();
                    } else {
                        showOperationSucToast(true);
                    }
                    break;
                    ...
                    if (msg.getData().getBoolean(RESULT)) {
                        if (msg.getData().getBoolean("haveNewVersion", false)) {
                            ...
                            if (!msg.getData().getBoolean(INIT_CHECKED)) {
                                Toast.makeText(BaseActivity.this, getString(R.string.check_back), Toast.LENGTH_SHORT).show();
                            }
                        }
                    } else {
                ...
                case QiupuMessage.MESSAGE_SEND_MAIL_END: {
                    ...
                    boolean ret = msg.getData().getBoolean(RESULT, false);
                    if (ret == false) {
                        Toast.makeText(BaseActivity.this, getString(R.string.send_message_failed), Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(BaseActivity.this, R.string.send_message_successful, Toast.LENGTH_SHORT).show();
                        onEmailInvited();
                    }
...
//            	showDialog(DIALOG_REMOVE_USER, data);
                ...
                case REQUEST_PROFIEL_END: {
                    ...
                    if (ret) {
                        long uid = msg.getData().getLong("uid");
                        Toast.makeText(BaseActivity.this, getString(R.string.request_ok), Toast.LENGTH_SHORT).show();
                    } else {
                        showOperationFailToast("", true);
                    }
                    break;
                ...
                okbtn.setOnClickListener(new OnClickListener() {
                    public void onClick(View arg0) {
                        ...
                        if (nameString.length() <= 0 || typeString.length() <= 0) {
                            Toast.makeText(BaseActivity.this, getString(R.string.invite_dialog_toast), Toast.LENGTH_SHORT).show();
                        } else {
                ...
                return new AlertDialog.Builder(this)
                        ...
                        .setItems(iteminfo, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                Intent intent = new Intent(BaseActivity.this, AddProfileInfoActivity.class);
                                if (which == 0) {
                                ...
                                if (textString.length() > 0) {
                                    ...
                                    for (int i = 0; i < size; i++) {
                                        ...
                                        if (tmpCircle != null && tmpCircle.name != null && tmpCircle.name.equals(textString)) {
                                            hasCirecle = true;
                                            Toast.makeText(BaseActivity.this, getString(R.string.circle_exists), Toast.LENGTH_SHORT).show();
                                            break;
                                        }
                                    }
                                    ...
                                    Toast.makeText(BaseActivity.this, getString(R.string.input_content), Toast.LENGTH_SHORT).show();
                                }
...
//                .setMessage(forceUpdate ? R.string.version_outdate_msg : R.string.version_update_msg)
                        ...
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                ...
                                if (currentApk == null) {
                                    ...
                                    //build for qiupu
                                    parseQiupuApkInfo(currentApk, BaseActivity.this.getApplicationInfo());
    ...
    private void forceExitProcess() {
        ...
        try {
            ...
            Intent accountservice = new Intent(BaseActivity.this, BorqsAccountService.class);
...
//    }
    ...
    private void shootFriendActivity(int current_type) {
        if (ensureAccountLogin()) {
            ...
            bundle.putString(BaseActivity.USER_NICKNAME, getUserNickname());
    ...
    protected void shootNotificationActivity() {
        if (ensureAccountLogin()) {
            Intent intent = new Intent(BaseActivity.this, BpcInformationActivity.class);
...
//        IntentUtil.startPeopleSearchIntent(this);
    ...
    protected OnClickListener composeStreamListener = new OnClickListener() {
        @Override
        public void onClick(View v) {
            IntentUtil.startComposeIntent(BaseActivity.this);
        }
    };
